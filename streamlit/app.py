    ...
    if video_file is not None or video_url is not None:
        if analyze_btn:
            ...
            if resp and resp.status_code == 200:
                ...
                if analyze_resp and analyze_resp.status_code == 200:
                        ...
                        while True:
                            ...
                            if status_resp and status_resp.status_code == 200:
                                ...
                                if status_data["status"] == "SUCCESS":
                                    ...
                                    status_text.error(f"❌ 分析失败: {status_data.get('error', '未知错误')}")
                                    break
                            else:
                                status_text.warning("获取状态失败，重试中...")
...
with tab2:
    if "current_video_id" not in st.session_state:
        ...
        if results_resp and results_resp.status_code == 200:
            ...
            with col_vid:
                st.subheader("🎥 视频播放")
                video_path_resp = api_request("GET", f"/videos/{video_id}/info")
